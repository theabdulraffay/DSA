...
class SegmentedTree {
	...
	private Node constructTree (int[] arr, int start, int end) {
		if (start == end) { // At this point we are at the leaf node, and its data is same as that of the arr index
		...
		node.data = node.left.data + node.right.data; // we collect data from left and right ans then add them to the current node, in this way the current node has the sum of array between 'start' and 'end' index
		return node;
	}
	...
	private int update(Node node, int index, int value) {
		...
		// keep calling recursion calls until you find the index with value and then update the value 
